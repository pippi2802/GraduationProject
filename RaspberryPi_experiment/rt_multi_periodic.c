...
int main(int argc, char **argv) {
    ...
    // Align release to a future absolute boundary so all processes & threads
    // start contending simultaneously. 5 s is plenty of headroom even when
    // launching two processes from a shell script.
    int64_t now = ns_now(CLOCK_MONOTONIC);
    int64_t aligned_start = ((now / 1000000000LL) + 5) * 1000000000LL;
