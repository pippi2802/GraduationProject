...
echo "Task 1: Period=100ms, WCET=10ms, Priority=1"
./periodic_task -C 10000 -p 100000 -P 1 -N 50 > "$OUTPUT_DIR/task1.txt" &
PID1=$!

# Task 2: Medium priority, medium period (200ms), 25 iterations
echo "Task 2: Period=200ms, WCET=15ms, Priority=2"
./periodic_task -C 15000 -p 200000 -P 2 -N 25 > "$OUTPUT_DIR/task2.txt" &
PID2=$!

# Task 3: Low priority, long period (300ms), 17 iterations
echo "Task 3: Period=300ms, WCET=20ms, Priority=3"
./periodic_task -C 20000 -p 300000 -P 3 -N 17 > "$OUTPUT_DIR/task3.txt" &
PID3=$!
