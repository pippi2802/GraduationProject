...
import sys, yaml
c = yaml.safe_load(open(f"workloads/{sys.argv[1]}/config.yaml"))
...
GLOB="workloads/$MODEL/$GEN_DIR/${SCALE:+$SCALE/}"
[ -n "$SCALE" ] && GLOB="workloads/$MODEL/$GEN_DIR/$SCALE" || GLOB="workloads/$MODEL/$GEN_DIR"
...
place_fixed_competitor() {
  ...
  if [ "$COMPETITOR_TYPE" = "unreserved" ]; then
    local intf="workloads/$MODEL/$GEN_DIR/_intf/$scale/$first_ul.yaml"
    ...
    FIXED_COMP_FILE="workloads/$MODEL/$GEN_DIR/_comp/$scale/$first_ul.yaml"
...
import json
try:
    d = json.load(open('workloads/$MODEL/$TAB_NAME'))
...
" 2>/dev/null)
  ...
  for cell_attempt in $(seq 1 "$CELL_ATTEMPTS"); do
    ...
    if [ "$HAS_NB" = 1 ]; then
      desired_target_cpu=""
      nb_file="workloads/$MODEL/$GEN_DIR/_nb/$scale/$ul.yaml"
