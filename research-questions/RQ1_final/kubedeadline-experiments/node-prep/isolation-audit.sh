...
pct_change() {
  ...
  awk -v b="$b" -v a="$a" 'BEGIN{
    if (b !~ /^-?[0-9.]+$/ || a !~ /^-?[0-9.]+$/) { print "n/a"; exit }
    if (b+0==0) { if (a+0==0) print "0 (both zero)"; else print "n/a (from zero)"; exit }
    printf "%.1f%%", (a-b)/b*100
  }'
}
...
do_report() {
  ...
  echo
  echo "NUMA topology (informational, no before/after): $OUTDIR/$label_b.numactl.txt"
  echo
