...
via placement retries) · `COLOCATE=1` (model2: retry until a reserved neighbour lands on
the target's SMT sibling) · `OUT_TAG` (results subdir suffix, e.g. `_sep` → `results/<model>_sep/`).
